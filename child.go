// This is a fork of the go library found at:
// https://code.google.com/p/go/source/browse/src/pkg/net/http/fcgi/child.go
// It reuses more objects and removes a lot of spawns of goroutines
...
func ( cp *ConnectionPool ) work ( jobId int ) {
  cp.wg.Add(1)
  defer cp.wg.Done()
  c := &child{
                //conn:     nil,//newConn(rwc),
  conn: &conn{rwc: nil},
                handler:  cp.handler,
                requests: make(map[uint16]*request),
        }

  reqClear := func( ic *child ) {
    for k := range ic.requests {
      delete( ic.requests, k )
    }
  }
  reqClear(c)
